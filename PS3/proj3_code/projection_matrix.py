...
def projection(P: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
    ...
    ##############################
    # TODO: Student code goes here
    first_dim = np.shape(points_3d)[0]
    projected_points_2d = np.zeros((first_dim, 2))
    for _ in range(first_dim):
        x_ = np.sum((P * np.append(points_3d[_][:], 1))[0], axis = 0)
        y_ = np.sum((P * np.append(points_3d[_][:], 1))[1], axis = 0)
        z_ = np.sum((P * np.append(points_3d[_][:], 1))[2], axis = 0)
        x_2d = x_ / z_
        y_2d = y_ / z_
        projected_points_2d[_][0] = x_2d 
        projected_points_2d[_][1] = y_2d 
    ##############################
...
def estimate_camera_matrix(pts2d: np.ndarray,
    ...
    ls = least_squares(objective_func, initial_guess, method='lm', verbose=2, max_nfev=5000, kwargs=kwargs).x
    ls = np.reshape(np.append(ls, 1), (3,4))
    P = ls
    ##############################
...
def decompose_camera_matrix(P: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # TODO: Student code goes here
    P_firstThree = P[:, :3]
    K, R = rq(P_firstThree) #first three columns
    ##############################
...
def calculate_camera_center(P: np.ndarray,
    ...
    # TODO: Student code goes here
    P_firstThree = P[:, :3]
    P_firstThree = P_firstThree * -1
    cc = np.dot(np.linalg.inv(P_firstThree), P[:, -1])
    ##############################
