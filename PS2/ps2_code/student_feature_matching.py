...
def match_features(features1, features2):
    ...
    confidences = np.empty((0,1)) #empty
    ...
    nndr = sorted_dist[:, 0] / (sorted_dist[:, 1] + 1e-10)

    confidences = nndr[nndr <= threshold]
    matches = np.stack([np.argwhere(nndr <= threshold).flatten() , sorted_idx[nndr <= threshold, 0].flatten()], axis=1)
...
def pca(fvs1, fvs2, n_components= 24):
    ...
    You will want to perform PCA on all the data toher to obtain the same principle components.
